...
function list_sessions_txt ()
{
  ...
  printf "+---------------------------+------------+----------+----------------------------+\n"
  for i in $(grep -E 'SESSION:' "${WORKDIR}"/sessions.txt 2>/dev/null | grep 'started' | awk '{print $2}' | sort | uniq); do

    # Load variables
    SIZE=$(du -h "${WORKDIR}/${i}" 2>/dev/null | awk '{print $1}')
    OPT=$(echo "${i}" | cut -d"-" -f1 )
...
function list_sessions_sqlite3 ()
{
  ...
  printf "+---------------------------+--------------+--------------+----------+----------------------------+\n"
  sqlite3 "${WORKDIR}"/sessions.sqlite3 'select * from backup_session' 2>/dev/null | while read -r i ; do
